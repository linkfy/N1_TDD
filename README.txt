...
Step 293) VALIDATION: CPU program can read controller bits from $4016
	Files:
		tests/chapter_07_controller_input/test_293_VALIDATION_cpu_reads_controller_4016.py

	Behavior:
		Build a tiny CPU program that writes 1 then 0 to $4016, reads controller bits
		from $4016, and stores observed bits into CPU RAM.

	Goal:
		validate the integration path:
			CPU instructions -> CpuBus $4016 -> Controller -> CPU RAM result
	...
	Important:
		This is a VALIDATION test. It should not introduce a new API.
