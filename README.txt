...
Step 295) Add manual pygame background display to main.py
	Files:
		main.py
		tests/chapter_08_manual_main/test_295_manual_main_pygame_background_display.py

	Behavior:
		Use pygame in main.py only to display the Framebuffer returned by
		Console.render_background_framebuffer() after each frame step.

	Goal:
		start feeling the manual "it works" loop visually while keeping pygame outside
		the emulator core.
	...
	Important:
		...
		Automated tests must not call main() or open a real pygame window.
	...
	After this:
		Step 296) Connect pygame keyboard events to controller_1 booleans in main.py.
		Step 297) Decide whether to improve background scrolling or start sprite rendering.
